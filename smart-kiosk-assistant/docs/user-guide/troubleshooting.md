...
For `openvino + NPU`, startup uses the same checked-in
`docker-compose.yml` plus a temporary runtime override generated by the
Makefile under `/tmp` to inject explicit `/dev/accel/accel*` mapping for
`audio-analyzer`. This temporary file is removed automatically and is not
tracked by git.
...
- `openvino + NPU` requires Intel NPU detection
