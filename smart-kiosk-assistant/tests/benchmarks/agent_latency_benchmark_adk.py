#!/usr/bin/env python3
"""End-to-end agent latency benchmark for the Smart AI Kiosk — ADK agent.

Hardcoded twin of ``agent_latency_benchmark.py`` that always targets the
production ADK-based ordering agent (``POST
rag-service:8020/api/v1/agent/chat``). Use ``agent_latency_benchmark_no_adk.py``
to run the identical benchmark against the standalone lab-style non-ADK
agent (``/api/v1/agent/chat-no-adk``) for a head-to-head A/B — see
...
Usage::
    ...
    python tests/benchmarks/agent_latency_benchmark_adk.py --label adk-baseline

    # Agent tier only (fast iteration loop), stack already running
    python tests/benchmarks/agent_latency_benchmark_adk.py --tier A --runs 10 --warmup 3 --skip-up
