#!/usr/bin/env python3
"""Replay every recorded conversation through the live agent (text-only, fast) — ADK agent.

Hardcoded twin of ``replay_all_conversations.py`` that always targets the
production ADK-based ordering agent (``/api/v1/agent/chat``). Use
``replay_all_conversations_no_adk.py`` to run the identical correctness
sweep against the standalone lab-style non-ADK agent
(``/api/v1/agent/chat-no-adk``) — see
...
Usage::
    ...
    python tests/benchmarks/replay_all_conversations_adk.py
    python tests/benchmarks/replay_all_conversations_adk.py --pattern 'conversations/2*.jsonl'
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    out = args.out or RESULTS_DIR / "replay-all-conversations-adk.json"
