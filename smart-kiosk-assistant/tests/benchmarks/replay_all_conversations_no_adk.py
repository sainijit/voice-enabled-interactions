#!/usr/bin/env python3
"""Replay every recorded conversation through the live agent (text-only, fast) — non-ADK agent.

Hardcoded twin of ``replay_all_conversations.py`` that always targets the
standalone lab-style non-ADK ordering agent (``/api/v1/agent/chat-no-adk``,
``OrderingAgentWithoutADK`` in ``plugins/kiosk/ordering_agent_without_adk.py``)
instead of the production ADK agent. Use ``replay_all_conversations_adk.py``
to run the identical correctness sweep against the production ADK agent
(``/api/v1/agent/chat``) — see
``docs/reports/ordering-agent-adk-vs-directive-2026-09.md``.
...
latency), this script hits ``rag-service``'s agent-chat endpoint directly
...
Usage::
    ...
    python tests/benchmarks/replay_all_conversations_no_adk.py
    python tests/benchmarks/replay_all_conversations_no_adk.py --pattern 'conversations/2*.jsonl'
"""
...
AGENT_CHAT_URL = "http://localhost:8020/api/v1/agent/chat-no-adk"
...
def main(argv: list[str] | None = None) -> int:
    ...
    out = args.out or RESULTS_DIR / "replay-all-conversations-no-adk.json"
