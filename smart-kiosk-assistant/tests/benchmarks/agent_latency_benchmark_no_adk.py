#!/usr/bin/env python3
"""End-to-end agent latency benchmark for the Smart AI Kiosk — non-ADK agent.

Hardcoded twin of ``agent_latency_benchmark.py`` that always targets the
standalone lab-style non-ADK ordering agent (``POST
rag-service:8020/api/v1/agent/chat-no-adk``, ``OrderingAgentWithoutADK`` in
``plugins/kiosk/ordering_agent_without_adk.py``) instead of the production
ADK agent. Use ``agent_latency_benchmark_adk.py`` to run the identical
benchmark against the production ADK agent (``/api/v1/agent/chat``) for a
head-to-head A/B — see
...
Usage::
    ...
    python tests/benchmarks/agent_latency_benchmark_no_adk.py --label no-adk-baseline

    # Agent tier only (fast iteration loop), stack already running
    python tests/benchmarks/agent_latency_benchmark_no_adk.py --tier A --runs 10 --warmup 3 --skip-up
...
RAG_AGENT_URL = "http://127.0.0.1:8020/api/v1/agent/chat-no-adk"
