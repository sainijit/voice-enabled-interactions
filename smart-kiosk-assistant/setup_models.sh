...
#
#   3. Silero VAD — kiosk-core's model-based voice-activity detector (~2.2 MB)
#                  Downloaded from the official snakers4/silero-vad repo.
#                  Placed in kiosk_core/models/silero_vad.onnx and baked into
#                  the kiosk-core Docker image at build time (COPY . . in the
#                  Dockerfile) -- same reasoning as YOLO26n above: it must
#                  exist on disk before `docker compose build`/`make build`.
#
...
SILERO_VAD_MODEL_PATH="${SCRIPT_DIR}/kiosk_core/models/silero_vad.onnx"
SILERO_VAD_URL="https://github.com/snakers4/silero-vad/raw/master/src/silero_vad/data/silero_vad.onnx"
# SHA256 of the v5 ONNX pinned above -- guards against a corrupt/partial
# download or an unexpected upstream change silently landing in the image.
...
# ── Silero VAD model (kiosk-core) ─────────────────────────────────────────────
download_silero_vad_model() {
    print_section "Silero VAD model → ${SILERO_VAD_MODEL_PATH}"

    if [ -f "${SILERO_VAD_MODEL_PATH}" ]; then
        local existing_sha256
        existing_sha256="$(sha256sum "${SILERO_VAD_MODEL_PATH}" | cut -d' ' -f1)"
        if [ "${existing_sha256}" = "${SILERO_VAD_SHA256}" ]; then
            echo "  ✓ silero_vad.onnx already present and verified — skipping"
            return 0
        fi
        echo "  ⚠  silero_vad.onnx present but checksum mismatch — re-downloading"
    fi

    mkdir -p "$(dirname "${SILERO_VAD_MODEL_PATH}")"
    echo "  Downloading silero_vad.onnx (v5) from snakers4/silero-vad..."
    curl -fSL "${SILERO_VAD_URL}" -o "${SILERO_VAD_MODEL_PATH}"

    local downloaded_sha256
    downloaded_sha256="$(sha256sum "${SILERO_VAD_MODEL_PATH}" | cut -d' ' -f1)"
    if [ "${downloaded_sha256}" != "${SILERO_VAD_SHA256}" ]; then
        echo "  ✗ Checksum mismatch for silero_vad.onnx"
        echo "    expected: ${SILERO_VAD_SHA256}"
        echo "    got:      ${downloaded_sha256}"
        rm -f "${SILERO_VAD_MODEL_PATH}"
        exit 1
    fi
    echo "  ✓ silero_vad.onnx downloaded and verified → ${SILERO_VAD_MODEL_PATH}"
}
